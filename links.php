...
<p>Before we go any further, we need to discuss a very important topic that we have neglected. This is URLs (Uniform Resource Locator). If you have ever browsed the web, then you have worked with URLs, even if you didn't know it. We usually call these "links", "sites", "pages", etc. It's all of the text in the bar at the top of your web browser (on mobile it's often hidden until you pull it up). Basically, what it does is it tells the browser where to go on the internet to get something and then show it to you. URLs have five basic components:
<ul>
...
<p>When specifying a URL, you can start at any segment, but you must declare segments in order (you can't say the host after the path it must be the host and then path). However, there are a few things to note:
<ul>
<li>The schema will default to "http://" unless "//" is used, then it will default to the current schema</li>
<li>The host will default to the current host unless specified</li>
<li>If the schema is specified the host needs to be specified as well</li>
...
<li>If the query and/or fragment is all that's specified, the browser will use the current page as the destination (ex. "?test" gives "<current page url>?test")</li>
</ul>
<h1>Link Tag</h1>
...
<pre>
&lt;<var>tag name</var> <var>property_name</var>="<var>property_value</var>" ...&gt;
</pre>
<p>The <var>tag name</var> is the name of the tag, the <var>property_name</var> is the name of the property, the <var>property_value</var> is the value of the property. The "..." means that a tag can have more than one property. This is great if we have a lot of extra information for the browser.
