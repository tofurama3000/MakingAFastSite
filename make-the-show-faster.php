...
<p>We already discussed the <code>async</code> property for JavaScript. Unfortunantely, we don't have that option for CSS. This means we have a few options:
<ul>
...
<p>This doesn't do too much, but it's by far the easiest. Basically, you take the CSS that you're going to send and have it be taken from multiple requests to a single request. It probably won't be done in one response, but it'll probably be faster than 5 or 6 distict requests.
...
<p>Another issue is you are deferring the loading of your CSS. Your page is loading without CSS - except the inline stuff. The solution (which will be the case for the all solutions that really help with page speed) is to prioritize which CSS should be loaded initially and be render blocking, and which CSS should be deferred for loading. This can be realatively easy if you're pages are so big that the footer is never visible, or you have elements that are hidden until user interaction; you just don't load all the CSS that makes those pretty initially (but do load enough that hidden elements are't visible).
<p>Overall, it's a good choice to defer your CSS loading, but it does require some consideration in how you do it, and what you do it to.
...
<p>If you have media queries in your CSS, it's fairly easy to split it out. You probably have a series of <code>print</code> elements, and if you're site is responsive you probably have some CSS based off of different screen sizes. You would then take the CSS for each media query, but it in it's own file, and then add the same query in the media property in the link tag. The browser will then look, and only block rendering for resources that are required for current media matches; otherwise it will work on that CSS when it's not doing something critical - like working on the critical rendering path.
<p>Ex. Lets say I had the following:
<pre>
media print{
	color: red;
}
